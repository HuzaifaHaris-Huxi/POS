...
# --------------------------------
def _business_logo_upload_to(instance, filename: str) -> str:
    now = timezone.localtime()
    return f"businesses/{instance.code}/logos/{now.strftime('%Y%m')}_{filename}"
...
class Business(CodeNamed):
    ...
    # Financial Onboarding & Subscription
    down_payment = models.DecimalField(**DECIMAL_12_2, default=Decimal("0.00"))
    monthly_subscription_fee = models.DecimalField(**DECIMAL_12_2, default=Decimal("0.00"))
    subscription_period_start = models.DateField(null=True, blank=True)
    subscription_period_end = models.DateField(null=True, blank=True)
    is_lifetime_subscription = models.BooleanField(default=False)
    
    # Branding
    logo = models.ImageField(upload_to=_business_logo_upload_to, null=True, blank=True)
    theme_color = models.CharField(max_length=20, blank=True, default="#000000")
...
class Branch(CodeNamed):
    ...
    def __str__(self):
        return f"{self.business.code} - {self.code} — {self.name}"


class BusinessSettings(TimeStampedBy):
    ...
    def __str__(self):
        return f"Settings: {self.business.code}"


class BranchSettings(TimeStampedBy):
    branch = models.OneToOneField(Branch, on_delete=models.CASCADE, related_name="settings")
    # Hardware
    pos_printer_name = models.CharField(max_length=200, blank=True, help_text="Thermal Printer Name/IP")
    barcode_scanner_config = models.CharField(max_length=255, blank=True, default="")
    cash_drawer_enabled = models.BooleanField(default=True)
    # Invoicing
    receipt_template = models.CharField(max_length=50, blank=True, default="default")
...
class BusinessSummary(models.Model):
    ...
    cash_in_hand = models.DecimalField(**DECIMAL_12_2, default=Decimal("0.00"), help_text="Total Cash in Hand across all branches")
    bank_balance = models.DecimalField(**DECIMAL_12_2, default=Decimal("0.00"), help_text="Total Bank Balance across all branches")
    inventory_value = models.DecimalField(**DECIMAL_12_2, default=Decimal("0.00"), help_text="Total Inventory Value across all branches")
    ...
    def aggregate_from_branches(self):
        """Recalculates this Business Summary based on all its active Branch Summaries."""
        from django.db.models import Sum
        
        aggregates = BranchSummary.objects.filter(branch__business=self.business).aggregate(
            total_rcv=Sum('total_receivables'),
            total_pay=Sum('total_payables'),
            total_cash=Sum('cash_in_hand'),
            total_bank=Sum('bank_balance'),
            total_inv=Sum('inventory_value')
        )
        
        self.total_receivables = aggregates['total_rcv'] or Decimal("0.00")
        self.total_payables = aggregates['total_pay'] or Decimal("0.00")
        self.cash_in_hand = aggregates['total_cash'] or Decimal("0.00")
        self.bank_balance = aggregates['total_bank'] or Decimal("0.00")
        self.inventory_value = aggregates['total_inv'] or Decimal("0.00")
        self.save()


class BranchSummary(models.Model):
    """
    Tracks the financial health metrics scoped to a specific branch.
    Aggregates up to the BusinessSummary.
    """
    ...
    def __str__(self):
        return f"Summary: {self.branch.name}"
    ...
    def save(self, *args, **kwargs):
        ...
        # Ping the parent business summary to recalculate
        try:
            business_summary = BusinessSummary.objects.get(business=self.branch.business)
            business_summary.aggregate_from_branches()
...
class Party(TimeStampedBy):
    ...
    # Scoping
    business = models.ForeignKey(Business, null=True, blank=True, on_delete=models.SET_NULL, related_name="parties")
    branch = models.ForeignKey(Branch, null=True, blank=True, on_delete=models.SET_NULL, related_name="parties")
    branch_display_index = models.PositiveIntegerField(null=True, blank=True)

    class Meta:
        indexes = [
            models.Index(fields=["type", "display_name"]),
            models.Index(fields=["branch", "branch_display_index"]),
        ]
...
class ProductCategory(TimeStampedBy):
    ...
    branch = models.ForeignKey(Branch, on_delete=models.PROTECT, related_name="product_categories", null=True, blank=True)
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=["branch", "code"]), models.Index(fields=["branch", "name"])]

    def __str__(self):
        code_part = self.code if self.code else "NO-CODE"
        b_code = self.branch.code if self.branch else self.business.code
        return f"{b_code}/{code_part}"

class Product(TimeStampedBy):
    business = models.ForeignKey(Business, on_delete=models.PROTECT, related_name="products")
    branch = models.ForeignKey(Branch, on_delete=models.PROTECT, related_name="products", null=True, blank=True)
    branch_display_index = models.PositiveIntegerField(null=True, blank=True)
    global_parent_product = models.ForeignKey('self', null=True, blank=True, on_delete=models.SET_NULL, related_name='branch_variants')
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["branch", "company_name"]),
        ]
...
class Staff(TimeStampedBy):
    ...
    # NEW: Multi-branch access for staff
    allowed_branches = models.ManyToManyField(
        Branch,
        blank=True,
        related_name="accessible_by_staff",
        help_text="Branches this staff member has active access to."
    )
    ...
    # Granular Permissions: Bank Accounts
    ...
    # Granular Permissions: Bank Movements
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["branch", "phone"],
                name="unique_phone_per_branch",
                condition=~models.Q(phone="")
            ),
            models.UniqueConstraint(
                fields=["branch", "cnic"],
                name="uniq_staff_branch_cnic_when_set",
                condition=~Q(cnic=""),
            ),
        ]
...
class BankAccount(TimeStampedBy):
    ...
    branch        = models.ForeignKey("Branch", on_delete=models.CASCADE, related_name="bank_accounts", null=True, blank=True, help_text="Branch this account belongs to.")
...
class CashFlow(TimeStampedBy):
    ...
    branch       = models.ForeignKey("Branch", on_delete=models.CASCADE, related_name="cash_flows", null=True, blank=True)
...
class BankMovement(TimeStampedBy):
    ...
    branch       = models.ForeignKey("Branch", on_delete=models.CASCADE, related_name="bank_movements", null=True, blank=True)
...
class StockTransaction(TimeStampedBy):
    ...
    branch     = models.ForeignKey("Branch", on_delete=models.PROTECT, related_name="stock_transactions", null=True, blank=True)
    date       = models.DateField(default=timezone.now, db_index=True)
...
class Payment(TimeStampedBy):
    ...
    branch         = models.ForeignKey("Branch", on_delete=models.PROTECT, related_name="payments", null=True, blank=True)
    date           = models.DateField(default=timezone.now, db_index=True)
...
class SalesInvoice(TimeStampedBy):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["branch", "invoice_no"]),
            models.Index(fields=["status"]),
            models.Index(fields=["branch", "branch_display_index"]),
        ]
...
# ============================
# INTERNAL PROCUREMENT & LEDGER
# ============================

class InternalTransfer(TimeStampedBy):
    class Status(models.TextChoices):
        PENDING    = "pending",    "Pending"
    ...
    total_transfer_cost = models.DecimalField(**DECIMAL_12_2, default=Decimal("0.00"), help_text="Total valuation of products being transferred")
    notes       = models.CharField(max_length=255, blank=True, default="")
    reference   = models.CharField(max_length=100, blank=True, default="")
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["status"]),
            models.Index(fields=["from_branch", "branch_display_index"]),
        ]

    def __str__(self):
        return f"Transfer #{self.pk or '—'} from {self.from_branch.code} to {self.to_branch.code}"

    def recompute_totals(self):
        total = Decimal("0.00")
        for item in self.items.all():
            total += (item.quantity or Decimal("0")) * (item.unit_cost or Decimal("0"))
        self.total_transfer_cost = total.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
...
class InternalTransferItem(models.Model):
    ...
    product_from = models.ForeignKey(Product, on_delete=models.PROTECT, related_name="transferred_out", help_text="Variant in sending branch")
    product_to   = models.ForeignKey(Product, on_delete=models.PROTECT, related_name="transferred_in", help_text="Variant in receiving branch")
    
    uom = models.ForeignKey(UnitOfMeasure, on_delete=models.PROTECT, null=True, blank=True)
    size_per_unit = models.DecimalField(max_digits=18, decimal_places=6, default=Decimal("1.000000"))
    
    quantity  = models.DecimalField(max_digits=18, decimal_places=6, validators=[MinValueValidator(0)])
    unit_cost = models.DecimalField(**DECIMAL_12_2, default=Decimal("0.00"), help_text="Valuation cost per unit at time of transfer")

    class Meta:
        ordering = ["id"]

    def __str__(self):
        return f"{self.quantity} x {self.product_from.name}"
    ...
    def save(self, *args, **kwargs):
        if not self.unit_cost or self.unit_cost == Decimal("0.00"):
            self.unit_cost = self.product_from.purchase_price or Decimal("0.00")
            
        super().save(*args, **kwargs)
        if self.transfer:
            self.transfer.recompute_totals()
            self.transfer.save()
...
class ActivityLog(models.Model):
    ...
    branch = models.ForeignKey(
        "Branch", # Assuming Branch is defined elsewhere or imported
        on_delete=models.SET_NULL,
        null=True, blank=True,
        related_name="activity_logs",
        help_text="The branch where this activity occurred"
    )
    staff_name = models.CharField(max_length=255, null=True, blank=True)
    staff = models.ForeignKey(
        "Staff", # Assuming Staff is defined elsewhere or imported
        on_delete=models.SET_NULL,
        null=True, blank=True,
        related_name="activity_logs",
    ...
    model_name = models.CharField(max_length=150, db_index=True, help_text="e.g., 'PurchaseOrder', 'Product'")
    record_id = models.CharField(max_length=50, null=True, blank=True, help_text="ID of the affected record")
    ...
    def __str__(self):
        return f"[{self.timestamp.strftime('%Y-%m-%d %H:%M')}] {self.staff_name or 'System'} {self.action} {self.model_name}"
