...
import { 
  Search, AddCircle, Edit, Delete, Category, Store
...
export default function Categories() {
  ...
  const fetchCategories = async (currentPage = page) => {
    ...
    try {
      ...
      const res = await axios.get(`${BACKEND_URL}/api/categories/?${params.toString()}`, { withCredentials: true });
      ...
      <div className="w-full h-full flex flex-col">
        ...
        <div className="p-6 border-b border-gray-50 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black text-gray-900 tracking-tight capitalize">Product Categories</h1>
              <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Catalog Organization</p>
            </div>
          </div>
          <button 
            onClick={openAddForm}
        ...
        <div className="flex-1 overflow-auto">
          ...
          <table className="w-full text-left">
            <thead className="sticky top-0 bg-white shadow-sm z-10">
              <tr>
                <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest bg-gray-50/80 backdrop-blur">Category Name</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr>
                  <td colSpan="4" className="p-8 text-center"><CircularProgress size={24} sx={{ color: '#EA5E28' }} /></td>
                </tr>
              ) : categories.length === 0 ? (
                <tr>
                ...
                categories.map(c => (
                  <tr key={c.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-bold text-gray-900 capitalize">{c.name}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-bold text-gray-600 uppercase">{c.code || '—'}</div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-center">
                      <Tooltip title="Edit" arrow>
                        <IconButton onClick={() => openEditForm(c)} size="small" sx={{ color: '#0EA5E9', '&:hover': { background: '#E0F2FE' }, mr: 0.5 }}>
                          <Edit fontSize="small" />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title="Delete" arrow>
                        <IconButton onClick={() => handleDelete(c.id)} size="small" sx={{ color: '#EF4444', '&:hover': { background: '#FEE2E2' } }}>
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50/50 flex items-center justify-between">
          <div className="text-xs font-bold text-gray-400">
            Showing {((page - 1) * limit) + (categories.length > 0 ? 1 : 0)} to {((page - 1) * limit) + categories.length} of {totalCount} entries
          </div>
