    ...
    const load = async () => {
        try {
            const [fd, od] = await Promise.all([
                ...
                axios.get(`${API}/api/purchase/${id}/`, { withCredentials: true })
            ]);
            ...
            setOrderDate(o.date || new Date().toISOString().split('T')[0]);
            setDiscPct(String(o.discount_percent));
            setTaxPct(String(o.tax_percent));
            setNotes(o.notes || '');
            setAlreadyPaid(toN(o.paid));
            setPaymentHistory(o.payments || []);
            
            const b = fd.data.branches?.find(x => x.name === o.branch) || fd.data.branches?.[0];
            setSelectedBranch(b);
            
            const v = fd.data.vendors?.find(vx => vx.display_name === o.vendor_name);
            ...
            setCart(o.items.map(i => ({
                ...
                sale_price: i.sale_price || 0, expiry_date: i.expiry_date || ''
            })));

            setExpenses(o.expenses.map(e => ({
                id: e.id, category: e.category, amount: String(e.amount), description: e.description || '', divide_per_unit: e.divide_per_unit
            })));
        } catch (e) {
            console.error('Load Error:', e);
            const msg = e.response?.data?.error || e.message || 'Failed to sync.';
            showToast('err', `Sync Error: ${msg}`);
        } finally {
            setLoading(false);
        }
    };
  ...
  const handleSave = async () => {
      ...
      try {
          const payload = {
              ...
              expenses: expenses.map(e => ({ id: e.id, category: e.category, amount: toN(e.amount), description: e.description, divide_per_unit: e.divide_per_unit })),
              payment: (payMethod !== 'credit' && toN(paidAmount) > 0) ? {
                  amount: toN(paidAmount),
                  payment_method: payMethod,
                  bank_account_id: payMethod === 'bank' ? selectedBank : null
              } : null
          };
          await axios.put(`${API}/api/purchase/${id}/edit/`, payload, { withCredentials: true });
          showToast('ok', 'Order synchronized successfully!');
      ...
      <div className="bg-white border-b border-gray-300 px-6 h-16 grid grid-cols-3 items-center shrink-0">
        ...
        <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
                ...
                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest leading-none">Record #PO-{id}</span>
            </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[300px] border-r border-gray-300 flex flex-col shrink-0 divide-y divide-gray-300/60 overflow-y-auto no-scrollbar">
          ...
          <div className="p-4 space-y-2">
             <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">A. Vendor</p>
          ...
          <div className="p-4 space-y-2">
             <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">B. Status</p>
          ...
          <div className="p-4 space-y-2">
             <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">C. Settlement</p>
             <div className="flex p-1 bg-gray-50 rounded-xl gap-1">
          ...
          {paymentHistory.length > 0 && (
            <div className="p-4 space-y-2">
               <p className="text-[8px] font-black text-gray-300 uppercase tracking-widest">D. Sync History</p>
               <div className="space-y-1 overflow-y-auto max-h-[120px] no-scrollbar">
                  {paymentHistory.map((p, i) => (
                    <div key={i} className="flex justify-between items-center p-2 bg-gray-50/50 rounded-lg border border-gray-50">
                       <div className="flex flex-col"><span className="text-[8px] font-black text-gray-700 uppercase">{p.payment_method}</span></div>
                       <span className="text-[10px] font-black text-emerald-600">Rs {fmtK(p.amount)}</span>
                    </div>
                  ))}
               </div>
            </div>
          ...
          <div className="p-4 pt-8 mt-auto space-y-3.5 bg-gray-50/30">
             <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 pb-3 mb-4">Sync Ledger</p>
             {[
             ...
             <div className="pt-2 mt-2 border-t border-gray-200 space-y-2 text-right">
                ...
                <div className="flex justify-between text-[10px] font-black uppercase text-gray-400 pt-2 tracking-widest border-t border-gray-50 mt-4">
                   <span>Paid: {fmtK(alreadyPaid + toN(paidAmount))}</span>
                   <span className={(netTotal - alreadyPaid - toN(paidAmount)) > 0.01 ? 'text-red-400' : ''}>Due: {fmtK(Math.max(0, netTotal - alreadyPaid - toN(paidAmount)))}</span>
                </div>
             </div>
          </div>
        </div>

        {/* MAIN AREA EDGE-TO-EDGE */}
              ...
              <div className="flex items-center gap-3 p-6 bg-gray-50/20 border-b border-gray-300 sticky top-0 bg-white z-[30]">
                 ...
                 <div className="w-[130px] shrink-0">
                    ...
                    <input ref={expiryRef} type="date" value={expiryDate} 
                      ...
                      className={`w-full h-10 px-3 bg-white border rounded-xl text-[11px] font-black outline-none shadow-sm transition-all ${!selectedProduct?.has_expiry ? 'bg-gray-100/50 border-transparent cursor-not-allowed opacity-40' : (expiryDate ? 'border-gray-200 focus:border-primary text-gray-600' : 'border-red-200 ring-2 ring-red-50/50 focus:border-red-400')}`} />
                 </div>
              ...
              <table className="w-full text-left truncate">
                ...
                <div className="p-8 space-y-10 border-t border-gray-300 bg-gray-50/5">
                   ...
                   <div className="space-y-4">
                      <p className="text-[10px] font-black text-gray-900 uppercase tracking-widest px-1">Ancillary Expenditures</p>
                      <div className="grid grid-cols-1 gap-1">
                         {expenses.map((ex, idx) => (
                           <div key={idx} className="flex gap-2 items-center">
                              <select value={ex.category} onChange={e => updateExpense(idx, 'category', e.target.value)} className="w-[110px] h-9 px-2 bg-white border border-gray-100 rounded-lg text-[9px] font-black uppercase outline-none">
                              ...
                              <input value={ex.description} onChange={e => updateExpense(idx, 'description', e.target.value)} placeholder="Entry details..." className="flex-1 h-9 px-4 bg-white border border-gray-100 rounded-lg text-[10px] font-bold outline-none" />
                              <div className="relative w-[140px]">
                                <input type="number" value={ex.amount} onChange={e => updateExpense(idx, 'amount', e.target.value)} className="w-full h-9 pl-6 pr-3 bg-white border border-gray-100 rounded-lg text-[11px] font-black text-right text-indigo-600 outline-none" />
                                <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[7px] font-black text-gray-300 uppercase leading-none">Rs</span>
                              </div>
                         ...
                         <button onClick={addExpense} className="w-fit mt-2 ml-1 text-[8px] font-black text-indigo-500 uppercase tracking-widest hover:underline">+ Append Charge Entry</button>
                      </div>
                   </div>

                   {/* Notes */}
                   <div className="space-y-3">
                      <p className="text-[10px] font-black text-gray-900 uppercase tracking-widest px-1">Modification Commentary</p>
                      <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Append rationale for record adjustment..."
          ...
          <div className="h-20 flex items-center justify-between px-10 border-t border-gray-300 shrink-0 bg-white z-20 shadow-[0_-10px_30px_rgba(0,0,0,0.04)] ring-1 ring-gray-300/40">
             ...
             <div className="flex items-center gap-6">
                ...
                <div className="flex items-center gap-4 bg-gray-50 p-1.5 rounded-2xl border border-gray-300">
                    <span className="text-[9px] font-black text-gray-400 uppercase px-4 tracking-widest">Update Record?</span>
