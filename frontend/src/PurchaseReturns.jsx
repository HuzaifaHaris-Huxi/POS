...
function ViewDrawer({ returnId, onClose, onStatusChanged }) {
  ...
  const load = () => {
    ...
    axios.get(`${API}/api/purchase/returns/${returnId}/`, { withCredentials: true })
  ...
  const changeStatus = async (s) => {
    ...
    setBusy(true);
    try {
      await axios.patch(`${API}/api/purchase/returns/${returnId}/status/`, { status: s }, { withCredentials: true });
    ...
    <>
      ...
      <div className="fixed top-0 right-0 bottom-0 w-[520px] bg-white z-[901] shadow-2xl flex flex-col">
        <div className="px-6 py-5 border-b border-gray-300 flex items-center justify-between shrink-0">
          <div>
            <p className="text-[9px] font-black text-gray-300 uppercase tracking-[0.2em] mb-1">Purchase Return</p>
          ...
          {ret && (
            <>
              <div className="grid grid-cols-2 gap-3">
                {[['Date', ret.created_at], ['Branch', ret.branch_name], ['Supplier', ret.vendor_name], ['Items', ret.items?.length || 0]].map(([l, v]) => (
                  ...
                  {ret.items?.map(item => (
                    <div key={item.id} className="flex items-center justify-between py-2 px-3 rounded-xl bg-gray-50 hover:bg-orange-50 transition-colors">
                      <div>
                        ...
                        <p className="text-[9px] text-gray-400 font-bold">qty {item.quantity} {item.uom_code}</p>
                      </div>
              ...
              <div className="bg-gray-50 rounded-2xl p-4 space-y-2.5">
                {[
                  { l: 'Total Return Value',  v: `Rs ${fmtK(ret.net_total)}`,  c: 'text-gray-900' },
                  { l: 'Refund Received',      v: `Rs ${fmtK(ret.refunded)}`,   c: 'text-emerald-600' },
                  { l: 'Remaining Balance',   v: `Rs ${fmtK(ret.balance)}`, c: ret.balance > 0 ? 'text-amber-600' : 'text-gray-300' },
                ].map(({ l, v, c }) => (
              ...
              {ret.status?.toUpperCase() !== 'CANCELLED' && (
                <div className="border border-gray-300 rounded-2xl p-4">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    <button disabled={busy} onClick={() => changeStatus('cancelled')}
                      className="px-4 py-1.5 rounded-xl border border-red-100 bg-red-50 text-red-600 text-[10px] font-black uppercase tracking-widest transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50">
                      Cancel Return
                    </button>
                  </div>
                  <p className="text-[8px] text-gray-300 font-bold mt-2">Processing decreases stock levels. Cancellation reverses all effects.</p>
                </div>
              )}
            </>
          )}
...
export default function PurchaseReturns() {
  ...
  const load = useCallback(async (pg = 1) => {
    setLoading(true); setError('');
    try {
      const p = new URLSearchParams({ offset: (pg - 1) * LIMIT, limit: LIMIT });
      ...
      const r = await axios.get(`${API}/api/purchase/returns/?${p}`, { withCredentials: true });
  ...
  const exportCSV = () => {
    const headers = ["Return #", "Date", "Vendor", "Branch", "Total", "Refunded", "Remaining", "Status"];
    ...
    link.setAttribute("download", `Purchase_Returns_Export_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const totalBal  = returns.reduce((s, o) => s + toN(o.balance), 0);
    ...
    <div className="h-full flex flex-col overflow-hidden animate-in fade-in duration-300" style={{ fontFamily: 'var(--font-google-sans, Google Sans, sans-serif)', background: '#F8FAFC' }}>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[9px] font-bold text-gray-300 uppercase tracking-widest leading-none">
              Stock Reversal (OUT) & Supplier Refunds
            </span>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        ...
        <div className="grid grid-cols-4 bg-white border-b border-gray-300 divide-x divide-gray-300 shrink-0 h-20">
          {[
            { l: 'Processed Returns', v: `${cntProc}`, s: 'Stock successfully reversed' },
            { l: 'Total Return Value', v: `Rs ${fmtK(totalNet)}`,    s: 'Supplier credits',   c: 'text-gray-900' },
            { l: 'Refunds Received',  v: `Rs ${fmtK(totalPaid)}`,   s: 'Cash/Bank inflows',   c: 'text-emerald-500' },
            { l: 'Net Outstanding',   v: `Rs ${fmtK(totalBal)}`,    s: 'Balance with suppliers',   c: totalBal > 0 ? 'text-amber-600' : 'text-gray-300' },
          ].map((k, i) => (
        ...
        <div className="h-14 px-8 border-b border-gray-300/80 flex items-center justify-between bg-gray-50/30 shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{total} Records</span>
          </div>
        </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={9} className="px-4 py-20 text-center">
                    <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.3em]">No purchase returns found</p>
                  </td>
                </tr>
              ) : returns.map((o) => (
                  <tr key={o.id} className="group border-b border-gray-300 transition-all hover:bg-orange-50/20">
                    ...
                    <td className="px-4 py-2.5 text-[11px] font-black text-gray-900">{o.vendor}</td>
                    ...
                    <td className="px-4 py-2.5 text-right text-[11px] font-black text-amber-600">Rs {fmtK(o.balance)}</td>
                    ...
                    <td className="px-4 py-2.5 flex items-center justify-center gap-2">
                      <button onClick={() => navigate(`/purchasing/returns/edit/${o.id}`)} className="w-7 h-7 flex items-center justify-center rounded-lg bg-white border border-gray-300 text-gray-400 hover:text-primary transition-all shadow-sm">
                        <Edit sx={{ fontSize: 14 }} />
                      </button>
