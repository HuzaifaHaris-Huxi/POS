...
import {
  ...
  Storefront, Add, Edit, Visibility, ReceiptLong
...
const STATUS_MAP = {
  OPEN:      { label: 'Open',      tw: 'bg-blue-50 text-blue-600 border-blue-100',              dot: 'bg-blue-400'    },
  PARTIAL:   { label: 'Partial',   tw: 'bg-amber-50 text-amber-600 border-amber-100',            dot: 'bg-amber-400'   },
  FULFILLED: { label: 'Fulfilled', tw: 'bg-emerald-50 text-emerald-600 border-emerald-100',      dot: 'bg-emerald-400' },
  CREDIT:    { label: 'Credit',    tw: 'bg-orange-50 text-primary border-orange-100',            dot: 'bg-primary'     },
...
function Badge({ status }) {
  const m = STATUS_MAP[status?.toUpperCase()] || STATUS_MAP.OPEN;
  return (
...
function ViewDrawer({ orderId, onClose, onStatusChanged }) {
  ...
  const load = () => {
    ...
    axios.get(`${API}/api/sales/${orderId}/`, { withCredentials: true })
  ...
  const changeStatus = async (s) => {
    if (!window.confirm(s === 'CANCELLED'
      ? `Cancel Order #${orderId}? Stock will be reversed and totals negated.`
      : `Change status to ${s}?`)) return;
    setBusy(true);
    try {
      await axios.patch(`${API}/api/sales/${orderId}/status/`, { status: s }, { withCredentials: true });
    ...
    <>
      ...
      <div className="fixed top-0 right-0 bottom-0 w-[520px] bg-white z-[901] shadow-2xl flex flex-col">
        <div className="px-6 py-5 border-b border-gray-300 flex items-center justify-between shrink-0">
          <div>
            <p className="text-[9px] font-black text-gray-300 uppercase tracking-[0.2em] mb-1">Sale Order</p>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-black text-gray-900 tracking-tight">{order ? `#${order.id}` : '…'}</h2>
          ...
          {order && (
            <>
              <div className="grid grid-cols-2 gap-3">
                {[['Date', order.created_at], ['Branch', order.branch], ['Customer', order.customer], ['Items', order.items?.length || 0]].map(([l, v]) => (
              ...
              <div>
                <p className="text-[9px] font-black text-gray-300 uppercase tracking-[0.2em] mb-3">Line Items</p>
                  ...
                  {order.items?.map(item => (
                    <div key={item.id} className="flex items-center justify-between py-2 px-3 rounded-xl bg-gray-50 hover:bg-orange-50 transition-colors">
                      <div>
                        ...
                        <p className="text-[9px] text-gray-400 font-bold">{item.uom_code} · qty {item.quantity}</p>
                      </div>
              ...
              <div className="bg-gray-50 rounded-2xl p-4 space-y-2.5">
                {[
                  { l: 'Sub Total',  v: `Rs ${fmtK(order.total_amount)}`,  c: 'text-gray-600' },
                  { l: `Discount (${order.discount_percent}%)`, v: `− Rs ${fmtK(order.total_amount * order.discount_percent / 100)}`, c: 'text-red-400' },
                ].map(({ l, v, c }) => (
                ...
                <div className="border-t border-gray-200 pt-2.5 flex justify-between">
                  <span className="font-black text-gray-900 text-sm">Net Total</span>
                  <span className="font-black text-primary text-lg">Rs {fmtK(order.net_total)}</span>
                </div>
                <div className="flex justify-between text-[11px]">
                  <span className="text-gray-400 font-bold">Received</span>
                  <span className="font-black text-emerald-600">Rs {fmtK(order.paid)}</span>
                </div>
                <div className="flex justify-between text-[11px]">
                  <span className="text-gray-400 font-bold">Balance Due</span>
                  <span className={`font-black ${order.balance_due > 0 ? 'text-red-500' : 'text-gray-300'}`}>Rs {fmtK(order.balance_due)}</span>
                </div>
              </div>
              {order.status?.toUpperCase() !== 'CANCELLED' && (
                <div className="border border-gray-300 rounded-2xl p-4">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {['OPEN', 'FULFILLED', 'CANCELLED'].filter(s => s !== order.status?.toUpperCase()).map(s => {
                      const m = STATUS_MAP[s] || STATUS_MAP.OPEN;
                      return (
                        <button key={s} disabled={busy} onClick={() => changeStatus(s)}
                          className={`px-4 py-1.5 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 ${m.tw}`}>
                          {s.charAt(0) + s.slice(1).toLowerCase()}
                        </button>
                      );
                    })}
                  </div>
                  <p className="text-[8px] text-gray-300 font-bold mt-2">Cancelling reverses stock and negates totals.</p>
                </div>
              )}
            </>
          )}
...
export default function SalesOrders() {
  ...
  const load = useCallback(async (pg = 1) => {
    ...
    try {
      ...
      const r = await axios.get(`${API}/api/sales/?${p}`, { withCredentials: true });
  ...
  const exportCSV = () => {
    const headers = ["Order #", "Date", "Customer", "Branch", "Net Total", "Paid", "Balance", "Status"];
  ...
  const totalNet  = orders.reduce((s, o) => s + toN(o.net_total), 0);
  const totalPaid = orders.reduce((s, o) => s + toN(o.paid), 0);
  const totalBal  = orders.reduce((s, o) => s + toN(o.balance_due), 0);
  const cntFulfil = orders.filter(o => o.status?.toUpperCase() === 'FULFILLED').length;
  const cntOpen   = orders.filter(o => ['OPEN', 'PARTIAL', 'CREDIT'].includes(o.status?.toUpperCase())).length;

  // ── skeleton rows
  const SkeletonRow = () => (
    <tr className="border-b border-gray-50 animate-pulse">
      {[40, 80, 100, 120, 90, 90, 90, 90, 60, 80].map((w, i) => (
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden animate-in fade-in duration-300" style={{ fontFamily: 'var(--font-google-sans, Google Sans, sans-serif)', background: '#F8FAFC' }}>
      ...
      <div className="bg-white border-b border-gray-300 px-8 h-16 grid grid-cols-3 items-center shrink-0">
        ...
        <div className="flex items-center gap-4">
          <div className="w-8 h-8 bg-primary/5 rounded-lg flex items-center justify-center">
            <ReceiptLong sx={{ fontSize: 18, color: '#EA5E28' }} />
          </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 space-y-0">
        ...
        {/* KPI Strip: High-Performance Ledger Style */}
        <div className="grid grid-cols-4 bg-white border-b border-gray-300 divide-x divide-gray-300 shrink-0 h-20">
          {[
            { l: 'Record Flow',       v: `${cntOpen} Pending | ${cntFulfil} Fulfill`, s: 'Lifecycle volume' },
            { l: 'Aggregate Revenue', v: `Rs ${fmtK(totalNet)}`,    s: 'Total net value',   c: 'text-gray-900' },
            { l: 'Liquidity In',      v: `Rs ${fmtK(totalPaid)}`,   s: 'Cash collections',     c: 'text-emerald-500' },
            { l: 'Pending Dues',      v: `Rs ${fmtK(totalBal)}`,    s: 'Outstanding credit',   c: totalBal > 0 ? 'text-red-500' : 'text-gray-300' },
          ].map((k, i) => (
        ...
        <div className="h-14 px-8 border-b border-gray-300/80 flex items-center justify-between bg-gray-50/30 shrink-0">
          <div className="flex items-center gap-6">
            ...
            { (dateFrom || dateTo) && (
              <button onClick={() => { setDateFrom(''); setDateTo(''); }} className="p-2 rounded-lg hover:bg-red-50 text-red-400 transition-colors">
          ...
          <div className="flex items-center gap-4">
             ...
             <button onClick={exportCSV}
               className="w-9 h-9 flex items-center justify-center rounded-xl bg-white border border-gray-300 text-gray-400 hover:text-emerald-600 hover:border-emerald-200 hover:shadow-md transition-all">
        ...
        {error && (
          <div className="bg-red-50 border border-red-100 rounded-2xl px-4 py-3 text-red-500 font-bold text-sm flex items-center gap-2">
        ...
        <div className="bg-white overflow-hidden flex-1 flex flex-col">
            ...
            <thead className="sticky top-0 z-20 bg-white">
              <tr className="bg-gray-50/50 border-b border-gray-300">
                {[
                  ...
                  { label: 'Net Total', align: 'right', w: 'w-32' },
                  { label: 'Paid', align: 'right', w: 'w-32' },
                  { label: 'Remaining', align: 'right', w: 'w-32' },
                  { label: 'Status', align: 'center', w: 'w-24' },
                  { label: 'Actions', align: 'center', w: 'w-20' },
                ].map(({ label, align, w }) => (
                  <th key={label}
                    className={`px-4 py-3 text-[9px] font-black text-gray-400 uppercase tracking-widest ${w}`}
                  ...
                  <td colSpan={11} className="px-4 py-20 text-center">
                    <div className="flex flex-col items-center gap-4">
                      <div className="p-5 bg-gray-50 rounded-3xl">
                        <ReceiptLong sx={{ fontSize: 40, color: '#E5E7EB' }} />
                      </div>
                      <div>
                        <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.3em]">No orders found</p>
                        <p className="text-[11px] text-gray-400 font-bold mt-1">Try a different period or branch filter</p>
                      </div>
                    </div>
                  </td>
                </tr>
              ) : orders.map((o, i) => {
                const isCxld = o.status?.toUpperCase() === 'CANCELLED';
                const rowCls = `group border-b border-gray-300 transition-all duration-150 ${isCxld ? 'opacity-40' : ''} hover:bg-orange-50/20`;
                const tdCls  = 'px-4 py-2.5 text-[11px] font-black text-gray-700';

                return (
                  <tr key={o.id} className={rowCls}>
                    {/* # */}
                    <td className={tdCls}>
                      <span className={`font-black tracking-tight ${isCxld ? 'text-gray-400' : 'text-primary'}`}>#{o.id}</span>
                    </td>
                    {/* Date */}
                    <td className={tdCls}>
                      <div className="flex items-center gap-1 text-gray-500">
                        <Schedule sx={{ fontSize: 11, color: '#D1D5DB' }} />
                        <span className="font-bold text-[11px] whitespace-nowrap">{o.created_at}</span>
                      </div>
                    </td>
                    {/* Party Name */}
                    <td className={tdCls}>
                    ...
                    {/* Branch */}
                    <td className={tdCls}>
                      <span className="text-gray-400 font-bold">{o.branch}</span>
                    </td>
                    {/* Net Total */}
                    <td className={`${tdCls} text-right`}>
                      <span className={`text-[12px] ${isCxld ? 'line-through opacity-50' : 'text-gray-900'}`}>Rs {fmtK(Math.abs(o.net_total))}</span>
                    </td>
                    {/* Paid */}
                    <td className={`${tdCls} text-right text-emerald-600`}>
                      Rs {fmtK(o.paid)}
                    </td>
                    {/* Remaining */}
                    <td className={`${tdCls} text-right`}>
                      <span className={o.balance_due > 0 ? 'text-red-500' : 'text-gray-400 opacity-30'}>Rs {fmtK(o.balance_due)}</span>
                    </td>
                    {/* Status */}
                    ...
                    <td className={`${tdCls} text-center`}>
                      <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => setViewId(o.id)}
                          className="w-7 h-7 flex items-center justify-center rounded-lg bg-white border border-gray-300 text-gray-400 hover:text-blue-500 hover:border-blue-200 transition-all shadow-sm">
                          <Visibility sx={{ fontSize: 14 }} />
                        </button>
                        {!isCxld && (
                          <button onClick={() => navigate(`/sales/edit/${o.id}`)}
                            className="w-7 h-7 flex items-center justify-center rounded-lg bg-white border border-gray-300 text-gray-400 hover:text-primary hover:border-orange-200 transition-all shadow-sm">
                            <Edit sx={{ fontSize: 14 }} />
                          </button>
                        )}
                      </div>
          ...
          {/*!loading && orders.length > 0 && (
            <div className="px-5 py-3 border-t border-gray-300 bg-gray-50 flex items-center justify-between">
              <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest">
                {(page - 1) * LIMIT + 1}–{Math.min(page * LIMIT, total)} of {total} records
              </p>
              <div className="flex items-center gap-1">
                <button disabled={page <= 1} onClick={() => setPage(p => p - 1)}
                  className="w-8 h-8 flex items-center justify-center rounded-xl border border-gray-300 bg-white text-gray-400 hover:border-primary/30 hover:text-primary disabled:opacity-30 disabled:cursor-not-allowed transition-all">
                ...
                {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                  ...
                  return (
                    <button key={pg} onClick={() => setPage(pg)}
                      className={`w-8 h-8 rounded-xl text-[10px] font-black transition-all ${page === pg ? 'bg-primary text-white shadow-premium' : 'bg-white border border-gray-300 text-gray-400 hover:border-primary/30 hover:text-primary'}`}>
                      {pg}
                    </button>
                  );
                })}
          ...
          {!loading && orders.length > 0 && (
            <div className="px-8 py-3 border-t border-gray-300 bg-white flex items-center justify-between shrink-0">
                <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.2em]">
                    {(page - 1) * LIMIT + 1}–{Math.min(page * LIMIT, total)} of {total} Records
                </p>
                <div className="flex items-center gap-1">
                    <button disabled={page <= 1} onClick={() => setPage(p => p - 1)}
                        className="w-8 h-8 flex items-center justify-center rounded-xl border border-gray-300 bg-white text-gray-400 hover:text-primary hover:border-primary/30 disabled:opacity-30 transition-all">
                        <ChevronLeft sx={{ fontSize: 18 }} />
                    </button>
                    <div className="flex items-center px-4 h-8 bg-gray-50 rounded-xl border border-gray-300 mx-2">
      ...
      {/* View Drawer */}
      {viewId && (
        <ViewDrawer orderId={viewId} onClose={() => setViewId(null)} onStatusChanged={() => load(page)} />
      )}
    </div>
  );
}
