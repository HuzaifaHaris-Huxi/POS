...
import { ArrowBack, Save, Warehouse } from '@mui/icons-material';

const BACKEND_URL = 'http://localhost:8000';

export default function WarehouseForm({ branches, warehouse, selectedBranch, onClose, onSaved }) {
    const isEdit = !!warehouse;
    const title = isEdit ? 'Edit Warehouse' : 'Create New Warehouse';
    ...
    useEffect(() => {
        if (warehouse) {
            setFormData({
                name: warehouse.name || '',
                code: warehouse.code || '',
                branch_id: warehouse.branch_id || (branches.length > 0 ? branches[0].id : ''),
    ...
    }, [warehouse, branches, selectedBranch]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEdit) {
                await axios.put(`${BACKEND_URL}/api/warehouses/${warehouse.id}/`, formData, { withCredentials: true });
            } else {
                await axios.post(`${BACKEND_URL}/api/warehouses/`, formData, { withCredentials: true });
            }
            onSaved();
        } catch (err) {
    ...
    return (
        <div className="h-screen flex flex-col bg-mesh p-6 lowercase text-sm">
            <div className="max-w-md mx-auto w-full bg-white rounded-3xl border border-gray-100 shadow-premium overflow-hidden flex flex-col">
                ...
                {/* Form Content */}
                <form id="warehouse-form" onSubmit={handleSubmit} className="p-8 space-y-6 flex-1 overflow-y-auto w-full hide-scrollbar">
                    {error && <div className="p-3 bg-rose-50 text-rose-500 rounded-xl font-bold text-xs">{error}</div>}
                    
                    <div className="space-y-4">
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Warehouse Name <span className="text-rose-500">*</span></label>
                            <input 
                                required
                                name="name"
                                value={formData.name}
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Warehouse Code <span className="text-rose-500">*</span></label>
                            <input 
                                required
                                name="code"
                                value={formData.code}
                        ...
                        <div className="space-y-1.5">
                            ...
                            <textarea 
                                ...
                                rows={3}
                                className="w-full px-4 py-2.5 rounded-xl border border-gray-100 focus:border-primary focus:ring-1 focus:ring-primary/20 outline-none transition-all font-bold placeholder:text-gray-300 capitalize resize-none"
                            />
                        </div>
                    </div>
                </form>
